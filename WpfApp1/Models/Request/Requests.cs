...
namespace WpfApp1
{
    public class Requests
    {
        ...
        private string GetResponseFromRequest(string url)
        {
        ...
        public User GetUser(string loginOrEmail, string password)
        {
            string json = GetResponseFromRequest($"http://f0773505.xsph.ru/DriveCarApi/?login={loginOrEmail}&password={password}");
        ...
        {
            string json = GetResponseFromRequest($"http://f0773505.xsph.ru/DriveCarApi/FreeCars");

            var cars = JsonSerializer.Deserialize<CarMessage>(json);

            return cars.Cars;
        }

        public List<Car> GetReservedCars(int userId)
        {
            string json = GetResponseFromRequest($"http://f0773505.xsph.ru/DriveCarApi/?userReservedCar={userId}");
        ...
        public void CarBooking(int userId, int carId, DateTime startDate, DateTime endDate, float totalPrice)
        {
            GetResponseFromRequest($"http://f0773505.xsph.ru/DriveCarApi/?userId={userId}&carId={carId}&startDate={startDate.ToString("yyyy-MM-dd")}&endDate={endDate.ToString("yyyy-MM-dd")}&totalPrice={totalPrice}");
        }
    }
}
